package de.asideas.overlay.controls.internal.views;
...
public class SurgeonFrameLayout extends FrameLayout
{
    ...
    private static final float ZOOM_DEFAULT = 0.85f;
    ...
    private final Map<View, Integer> layerVisibilities = new HashMap<View, Integer>();

    private final SparseArray<String> idNames = new SparseArray<String>();
    ...
    public void draw(@SuppressWarnings("NullableProblems") Canvas canvas)
    {
        ...
        while (!layeredViewQueue.isEmpty())
        {
            ...
            if (drawIds)
            {
                ...
                if (id != NO_ID)
                {
                    String name = "id: " + nameForId(id);
                    canvas.drawText(name, textOffset, textSize, viewBorderPaint);
                    canvas.translate(0, textSize + 2);
                    canvas.drawText("class: " + view.getClass().getSimpleName(), textOffset, textSize, viewBorderPaint);
                }
            }
        ...
        LayoutTraverser.build(new LayoutTraverser.Processor()
        {
            ...
            public void process(View view)
            {
                if (counter == MAX_LAYER_COUNT)
                {
                    ...
                    {
                        view.setVisibility(layerVisibilities.get(view));
                    }
                }
                else if (getLayerDistance(view) == counter)
                {
                    if (!layerVisibilities.containsKey(view))
                    {
                        layerVisibilities.put(view, view.getVisibility());
                    }
    ...
    private int getLayerDistance(View view)
    {
        ViewParent vp = view.getParent();
        int layer = 0;
        while (vp != null && vp != vp.getParent())
        {
            layer++;
            vp = vp.getParent();
        }
